...
export default async function JobDetailPage({
  ...
  const goodToHave = toList(job.goodToHave);
  const lookingFor = toList(job.lookingFor);
  const qualifications = toList(job.experience);
  ...
  const Section = ({
    ...
    <div className="space-y-2">
      <h4 className="text-sm font-bold tracking-widest uppercase text-black">
        {title}
      </h4>
      {children}
    </div>
  ...
  const List = ({ items }: { items: string[] }) => (
    <ul className="space-y-2 ">
      {items.map((item, i) => (
        <li
          key={i}
          className="flex gap-2 text-sm text-muted-foreground leading-relaxed items-center"
        >
  ...
  return (
    <div className="pt-16 lg:py-24">
      <div className="container mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* LEFT */}
          <div className="lg:col-span-2 space-y-6 bg-background border border-primary rounded-lg p-6 md:p-8">
            {/* Header */}
            <div className="flex items-center gap-4">
              <Image
                src={logo}
            ...
            {goodToHave.length > 0 && (
              <Section title="Good to Have">
                <List items={goodToHave} />
              </Section>
            )}

            {lookingFor.length > 0 && (
              <Section title="What We're Looking For">
                <List items={lookingFor} />
              </Section>
            )}
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
              <div className="bg-background border border-primary rounded-lg p-6">
                ...
                <div className="space-y-4 text-sm">
                  ...
                  {job.jobFunction && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">{job.jobFunction}</p>
                        <p className="text-xs text-muted-foreground">
                          Job function
                        </p>
                      </div>
                ...
                <Link
                  href={`/careers/jobs/${slug}/apply`}
                  className="block mt-6"
                >
                  <button className="w-full inline-flex items-center justify-center px-6 py-4 text-white rounded-full text-sm font-bold bg-primary hover:scale-105 transition-all duration-300 ease-out">
                    Apply for this job
                  </button>
                </Link>
              </div>
