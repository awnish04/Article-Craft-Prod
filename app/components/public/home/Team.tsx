...
const COLUMN_COUNT = 9;
const GAP_PX = 80;
const IMG_SIZE = 112; // w-28 = 112px
const PADDING_Y = 64; // py-16 = 64px
...
// ─── TeamRow — ONE useInView shared by both left + right images ───────────────

function TeamRow({ idx }: { idx: number }) {
  const ref = useRef(null);

  // Single ref at the vertical midpoint of this row.
  // Both images read from this ONE isInView — guaranteed to fire simultaneously.
  const isInView = useInView(ref, {
    margin: "-80px 0px -80px 0px",
    amount: 0,
  });

  const left = LEFT_COLUMN[idx];
  const right = RIGHT_COLUMN[idx];

  const topPx = PADDING_Y + idx * (IMG_SIZE + GAP_PX);
  const sharedTransition = { duration: 1, ease: "easeOut" } as const;

  return (
    <>
      {/* Invisible anchor at the vertical midpoint of this row */}
      <div
        ref={ref}
        className="absolute left-1/2 -translate-x-1/2 w-1 h-1 pointer-events-none"
        style={{ top: topPx + IMG_SIZE / 2 }}
      />

      {/* Left image */}
      <div
        className={`absolute left-0 pointer-events-none transition-transform ${left.mdOffsetClass} ${left.lgOffsetClass}`}
        style={{ top: topPx }}
      >
        <motion.div
          className="relative w-28 h-28 lg:w-32 lg:h-32 rounded-full overflow-hidden border border-primary shrink-0"
          animate={
            isInView
              ? { scale: 1, opacity: 1, x: 0, y: 0, rotate: 0 }
              : {
      ...
      {/* Right image */}
      <div
        className={`absolute right-0 pointer-events-none transition-transform ${right.mdOffsetClass} ${right.lgOffsetClass}`}
        style={{ top: topPx }}
      >
        <motion.div
          className="relative w-28 h-28 lg:w-32 lg:h-32 rounded-full overflow-hidden border border-primary shrink-0"
  ...
  return (
    <>
      ...
      >
        ...
        {/* Rows — left + right share one useInView trigger per row */}
        {Array.from({ length: COLUMN_COUNT }, (_, idx) => (
          <TeamRow key={idx} idx={idx} />
        ))}
      </section>
